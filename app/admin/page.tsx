...
export default function AdminPage() {
  ...
  return (
    <>
      {/* Contenedor principal de la página de administración, se oculta en móvil si el formulario está visible */}
      <div className={`min-h-screen bg-gray-50 ${showForm ? 'hidden sm:block' : 'block'}`}>
      ...
      <AnimatePresence>
        {showForm && (
          <motion.div
            key="combo-form-modal"
            className="fixed inset-0 z-50 flex flex-col bg-white sm:bg-black/50 sm:items-center sm:justify-center"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            <motion.div
              className="flex flex-col h-full w-full bg-white sm:h-auto sm:max-w-2xl sm:rounded-lg sm:shadow-2xl"
            ...
            >
              ...
              <div className="flex-shrink-0 flex justify-between items-center p-4 border-b">
                <h3 className="text-lg font-semibold">{editingCombo?.id ? 'Editar Combo' : 'Agregar Nuevo Combo'}</h3>
              ...
              {/* Footer del Modal */}
              <div className="flex-shrink-0 flex justify-end p-4 border-t bg-gray-50">
                <button
                  type="button"
                  onClick={() => setShowForm(false)}
                  className="mr-2 bg-gray-200 text-gray-800 px-4 py-2 rounded-md shadow-sm hover:bg-gray-300"
                >
                  Cancelar
                </button>
                <button
